...
Primary key : ORDID
Foreign keys : CUSTID,RID,PID,AGENTID */
CREATE TABLE ORD(
...
/*Create ORDERITEM table and inserting data.ORDERITEM table has four foreign keys and one primary key
Primary key : MID,ORDID
Foreign keys : MID,ORDID */
...
-- Find customers (custid, name) who ordered food in october and has address 4410 in it

Select customer.custid, customer.fname from customer inner join Ord on (customer.custid=ord.custid) where To_char(ODate, 'MM')='10' and lower(address) like '%4410%';
